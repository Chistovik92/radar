...
do_rollback() {       # do_rollback [путь к снимку]
    ...
    # Откат тоже проходит через `down --remove-orphans`, то есть уносит
    # Caddy. В 4.8.4.3 возврат добавили только на обычный путь установки,
    # и после неудачного обновления с откатом панель оставалась недоступной
    # при работающем боте — ровно та поломка, которую 4.8.4.3 и чинил.
    ensure_tls_running
