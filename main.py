...
RELEASES: list[tuple[str, list[str]]] = [
    ("5.9", [
        ...
        "историю. Установщик предлагает перенос сам, прежняя база остаётся "
        "на диске.",
    ]),
    ("5.8.1", [
    ...
    ("5.7", [
        ...
        "тревоги приходят во все связанные сети. Связать: /link в одной сети, "
        "код — в другой, подтвердить «да».",
        "📍 Начать можно и не в Telegram: во ВКонтакте и MAX адрес задаётся "
        "командой /address или геопозицией, в Discord — слеш-командой /address.",
        "🖥 Вход в веб-панель по одноразовому коду из /panel — в любой сети, "
        "и по IP-адресу тоже.",
    ]),
    ("5.6.2", [
    ...
    ("4.9.8.1", [
        ...
        "пропадало подменю — из такой страницы некуда было вернуться. "
        "Теперь раздел остаётся подсвеченным, а над заголовком стоит "
        "ссылка назад.",
    ]),
    ("4.9.8", [
    ...
    ("4.9.3.1", [
        ...
        "Код в репозитории был верным — устарел только встроенный "
        "в установщик экземпляр; теперь он собран заново и сверяется "
        "CI автоматически.",
        "👟 <b>Возвращён бит исполнения</b> у установщика и служебных "
        "скриптов: сброшен одним из коммитов с Windows. На запуск "
        "через «bash install.sh» он не влиял, но прямой запуск файла "
        "требовал лишнего действия.",
    ]),
    ("4.9.3", [
