...
на 127.0.0.1 и гоняет через них настоящие клиенты (с 5.8 — и привязку
чужих записей по списку клиентов). Ему нужен настоящий
...
кодом и «да» (с 5.7), адрес из ВК в общем профиле и копия тревоги.

`tools/upgrade_check.py` (5.8.1) — не обязательный, но нужный перед
выпуском, меняющим базу: код каждого старого тега (по умолчанию все
